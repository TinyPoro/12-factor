# 12-factor
## Giới thiệu
Trong thời đại ngày nay, phần mềm thường được  cung cấp dưới dạng dịch vụ, gọi là các ứng dụng web hoặc phần-mềm-như-1-dịch-vụ. Phần mềm theo 12-chuẩn là 1 phương pháp luận xây dựng phần-mềm-như-1-dịch-vụ:

- Sử dụng các định dạng được khai báo để cài đặt tự động, Giảm thiểu thời gian và chi phí cho các người phát triển mới tham gia vào dự án:
- Có các giao ước rõ ràng với hệ điều hành ngầm, cho phép tính di dộng tối đa giữa các môi trường thực thi.
- Phù hợp để triển khai (deployment) trên các nền tảng đám mây hiện đại, giảm bớt yêu cầu từ server và các hệ thống quản trị.
- Giảm thiểu sự khác nhau giữa development và production, cho phép phát triển liên tục 1 cách nhanh nhất.
- Và có thể mở rộng và không cần có những thay đổi đáng kể nào với công cụ, kiến trúc, hay thói quen phát triển.
...
## Background
Các tác giả đóng góp cho tài liệu đã áp dụng trực tiếp trong quá trình phát triển và triển khai hàng trăm ứng dụng, và  gián tiếp chứng kiến quá trình phát triển, điều hành và mở rộng của hàng trăm nghìn ứng dụng qua công việc trên nền tảng Heroku.
...
Động lực của chúng tôi là nâng cao nhận thức về vài vấn đề có hệ thống chúng tôi đã gặp trong phát triển ứng dụng hiện đại, để cung cấp các từ vựng chung để bàn luận về những vấn đề này, và để cho phép 1 tập các giải pháp khái niệm rộng lớn vào các vấn đề với các thuật ngữ kèm theo. Định dạn này lấy cảm hứng bởi cuốn sách của Martin Fowler, Patterns of Enterprise Application Architecture and Refactoring.
...
#### I. Codebase
###### 1 codebase được theo dõi trong việc kiểm soát các thay đổi, nhiều bản triển khai.
...
1 codebase có thể là bất cứ repo đơn lẻ nào nào (trong 1 hệ thống kiểm soát sửa đổi tập trung như Subversion), hoặc có thể lả 1 tập các repo chia sẻ chung 1 commit gốc(trong 1 hệ thống kiểm soát sửa đổi phân cấp như Git). 
...
1 ứng dụng theo 12-chuẩn không bao giờ phụ thuộc vào các tồn tại ngầm của các gói ở mức-hệ-thống. Nó khai báo tất cả các phụ thuộc, hoàn thiện và chính xác, thống qua 1 biểu thị khai báo phụ thuộc. Hơn nữa, nó sử dụng 1 công cụ tách biệt phụ thuộc trong quá trình thực thi để đảm bảo rằng không có phụ thuộc ngầm nào “lọt vào”  từ các hệ thống xung quanh. Đặc tả phụ thuộc đầy đủ và rõ ràng được áp dụng thống nhất cho cả productiont và developer.
...
1 lợi ích của việc khai báo phụ thuộc rõ ràng là nó đơn giản hóa việc cài đặt cho những người phát triển mới của ứng dụng. Người phát triển mới có thể lấy codebase của ứng dụng về máy phát triển của họ, với điều kiện tiên quyết chỉ yêu cầu ngôn ngữữ chạy và quản lý phụ thuộc được cài đặt. Họ sẽ cóó thể cài đặt bất cứ thứ gì cần thiết để chạy code của ứng dụng với 1 lệnh xây dựng xác định. Ví dụ, lấy xây dựng cho Ruby/Bundler là `bundle install`, còn với Clojureojure/Leiningen là (`lein deps`).
...
Các ứng dụng đôi khi lưu trữ cấu hình như các hằng số trong codeode. Điều này xung đột với 12-chuẩn, khi nó yêu cầu `tính tách biệt 1 cách nghiêm ngặt trong cấu hình ở code`. Cấu hình thay đổi  giữa các triển khai, còn code thì không.
...
Ứng dụng theo 12-chuẩn lưu trữ cấu hình trong các biến môi trường ( thường đọc tắt là env vars hoặc env). ENV vars thường dễ thay đổi giữa các triển khai mà không phải thay đổi code, không giống như các file cấu hình, chỉ 1 chút thay đổi của chúng được kiểm soát phụ thuocj bởi code repo; và không giống các file cấu hình thông thường,  các cơ chế cấu hình khác như Java System Properties, chúng là các chuẩn ngôn ngữ và OS-agnostic.
